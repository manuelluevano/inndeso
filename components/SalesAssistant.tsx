...
function getAnswer(question: string) {
  ...
  if (/plataforma|android|ios|iphone|windows|mac|watch|linux|servidor/.test(value)) return "Desarrollamos para web, Windows, macOS, iOS, Android y Apple Watch. También implementamos APIs, nube y servidores Linux instalados en sitio cuando el negocio requiere control local.";
  if (/inventario|crm|venta|caja|finanza|producto|pedido/.test(value)) return "Podemos integrar CRM, clientes, ventas, cotizaciones, productos, inventario, compras, caja, cobranza y reportes en una sola plataforma, con app móvil si la operación lo necesita.";
  ...
  if (/proyecto|caso|ejemplo|marca/.test(value)) return "Puedes explorar GeoJornada, Sana Life, MovilSource, CellGo, LibreMente y Planifica40 en la sección de proyectos. Ahí mostramos funciones, plataformas y pantallas del producto.";
